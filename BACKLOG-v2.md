---
title: "BACKLOG-v2 — Implementation-Ready Roadmap"
date: 2026-03-13
tags: [backlog, roadmap, architecture, planning]
superseded_by: ""
---

# BACKLOG-v2 — Implementation-Ready Roadmap

> This file supersedes the discovery-oriented `BACKLOG.md` as the active build plan.
> `BACKLOG.md` remains in place as a historical record of the research agenda.
...
┌────────────────────────────────────────────────────────────────┐
│                        CAPTURE LAYER                           │
│   iOS Shortcut │ CLI remember │ Telegram bot │ Copilot agent  │
│   Slack bot    │ inbox/ folder │ MCP clients (VS Code, Cursor) │
└────────────────────────────┬───────────────────────────────────┘
                             │  add_memory(title, content, folder)
┌────────────────────────────▼───────────────────────────────────┐
│                        MCP SERVER                              │
│   mcp_server.py — search_brain / add_memory / refactor_memory  │
│   stdio transport · pre-computed embeddings on write           │
└─────────────┬──────────────────────────────┬───────────────────┘
              │ write + embed                │ vector search
┌─────────────▼──────────┐    ┌─────────────▼─────────────────┐
│     STORAGE LAYER      │    │       RETRIEVAL LAYER          │
│   GitHub repo          │    │   LanceDB (serverless, local)  │
│   .md files            │    │   384-dim vectors              │
│   .embedding.json      │    │   Model2Vec or BGE-small-en    │
│   YAML front-matter    │    │   warm index from JSON files   │
│   git commit + push    │    │   cold-start < 2 s             │
└────────────────────────┘    └────────────────────────────────┘
```
...
|---|---|
| `2026-03-08-lancedb-index-rebuild-from-git.md` | W-0100, W-0101 |
| `2026-03-02-semantic-full-text-search.md` | W-0101 |
| `2026-03-08-inbox-folder-capture-triage-pattern.md` | W-0102, W-0103 |
| W-0008 (iOS Shortcut — still discovery-phase in BACKLOG.md) | W-0104 |
| W-0010 (CLI — still discovery-phase in BACKLOG.md) | W-0105 |
...
> No new features. Make what exists work correctly and at acceptable latency.
